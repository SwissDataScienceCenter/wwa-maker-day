...
sm_data = rp2.StateMachine(0, data_hub75, out_base=Pin(data_pin_start), sideset_base=Pin(clock_pin), freq=40000000)#55000000)
sm_row = rp2.StateMachine(1, row_hub75, out_base=Pin(row_pin_start), sideset_base=Pin(latch_pin_start), freq=40000000)#1000000)
...
rows = []
blocks_per_row = const(32) #each block has 24 bits, so 4 pixels x 2 scanlines
num_rows = const(16)
...
def draw_frog():
    ...
    text_y = text_y + direction
    if text_y > 19: direction = -0.4
    if text_y < 0: direction = 0.4
    
    text_x = text_x + direction_x
    if text_x > 115: direction_x = -0.4
    if text_x < 0: direction_x = 0.4
